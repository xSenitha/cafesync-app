...
export function StatCard({ label, value, icon, color }: StatCardProps) {
  return (
    <div className="bg-white p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border border-stone-100 shadow-sm flex items-center gap-3 sm:gap-5">
      <div className={`${color} p-3 sm:p-4 rounded-xl sm:rounded-2xl text-white shadow-lg shadow-stone-200`}>
        {icon}
      </div>
      <div>
        <p className="text-[9px] sm:text-[10px] font-black text-stone-400 uppercase tracking-widest">{label}</p>
        <p className="text-lg sm:text-xl font-black text-stone-800 mt-0.5">{value}</p>
      </div>
    </div>
  );
}
...
export function QuickAction({ icon, label, onClick, color }: QuickActionProps) {
  return (
    <button 
      onClick={onClick}
      className={`${color} p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] text-white flex flex-col items-center justify-center gap-2 sm:gap-3 hover:scale-105 transition-all shadow-lg active:scale-95 group`}
    >
      <div className="bg-white/20 p-2 sm:p-3 rounded-xl sm:rounded-2xl group-hover:bg-white/30 transition-colors">
        {icon}
      </div>
